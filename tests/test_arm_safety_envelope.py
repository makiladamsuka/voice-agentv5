...
class TestArmSafetyEnvelope(unittest.TestCase):
    ...
    def test_right_sweep_mid_raise(self) -> None:
        # A0=85.5 → t=0.5 between 47 and 124
        lo, hi = self.env.sweep_range(side="right", raise_deg=85.5)
        self.assertAlmostEqual(lo, 44.0)
        self.assertAlmostEqual(hi, 54.0)

    def test_right_sweep_decreases_as_raise_increases(self) -> None:
        _, hi_low = self.env.sweep_range(side="right", raise_deg=47.0)
        _, hi_mid = self.env.sweep_range(side="right", raise_deg=85.0)
        _, hi_high = self.env.sweep_range(side="right", raise_deg=124.0)
        self.assertGreater(hi_low, hi_mid)
        self.assertGreater(hi_mid, hi_high)

    def test_left_sweep_at_low_raise(self) -> None:
    ...
    def test_left_sweep_at_high_raise(self) -> None:
        lo, hi = self.env.sweep_range(side="left", raise_deg=0.0)
        self.assertAlmostEqual(lo, 70.0)
        self.assertAlmostEqual(hi, 70.0)

    def test_left_sweep_mid_raise(self) -> None:
        # A1=32.5 → t=0.5 between 65 and 0
        lo, hi = self.env.sweep_range(side="left", raise_deg=32.5)
        self.assertAlmostEqual(lo, 78.5)
        self.assertAlmostEqual(hi, 86.0)

    def test_left_sweep_decreases_as_raise_increases(self) -> None:
        _, hi_low = self.env.sweep_range(side="left", raise_deg=65.0)
        _, hi_mid = self.env.sweep_range(side="left", raise_deg=46.0)
        _, hi_high = self.env.sweep_range(side="left", raise_deg=0.0)
        self.assertGreater(hi_low, hi_mid)
        self.assertGreater(hi_mid, hi_high)
    ...
    def test_clamp_sweep_down_when_raise_increases(self) -> None:
        a0, a1, a2, a3 = self.env.clamp_arms(124.0, 65.0, 64.0, 87.0)
        self.assertAlmostEqual(a0, 124.0)
        self.assertAlmostEqual(a2, 44.0)

    def test_raise_saturates_above_high_anchor(self) -> None:
        lo, hi = self.env.sweep_range(side="right", raise_deg=124.0)
        self.assertAlmostEqual(lo, 44.0)
        self.assertAlmostEqual(hi, 44.0)
