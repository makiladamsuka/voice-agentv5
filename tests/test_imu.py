...
def run_level_calibrate(bus: int, address: int, seconds: float, *, write_config: bool) -> int:
    ...
    reader.start()
    print("Keep head at mechanical center (P78 T112) and hold still ...")
    time.sleep(0.5)
    samples = []
    deadline = time.time() + seconds
    try:
    ...
    roll = reader.filter.roll_offset_deg
    pitch = reader.filter.pitch_offset_deg
    print(
        f"Level offsets: roll {roll:+.3f}°, pitch {pitch:+.3f}° ({len(samples)} samples)"
    )
    if write_config:
        _write_imu_offsets_to_config(roll, pitch)
