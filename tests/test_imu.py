...
from imu_sensor import Bmi160, ImuAttitudeFilter, ImuReader
...
def _load_imu_defaults() -> tuple[int, int]:
    ...
    try:
        ...
        imu = cfg.get("imu") or {}
        bus = int(imu.get("i2c_bus", bus))
        address = int(imu.get("address", address))
    except Exception:
        pass
    return bus, address
...
def run_raw_stream(bus: int, address: int, seconds: float, hz: float) -> int:
    ...
    filt = ImuAttitudeFilter(axis_remap=_load_axis_remap())
    dev.open()
...
def run_level_calibrate(bus: int, address: int, seconds: float) -> int:
    reader = ImuReader(bus=bus, address=address, sample_hz=100.0)
    reader.start()
    print("Keep the head still for level calibration ...")
    time.sleep(0.5)
    samples = []
    deadline = time.time() + seconds
    try:
        ...
        reader.stop()
    if len(samples) < 40:
        print("Not enough IMU samples — check wiring and I2C address.")
        return 1
    reader.filter.calibrate_level(samples)
    print(
        f"Level offsets: roll {reader.filter.roll_offset_deg:+.3f}°, "
        f"pitch {reader.filter.pitch_offset_deg:+.3f}° "
        f"({len(samples)} samples)"
    )
    print("Add these to config if you persist offsets later.")
    return 0
...
def run_spin_test(port: str, baud: int, degrees: float, verify: bool) -> int:
    ...
    reader = ImuReader(sample_hz=100.0)
    reader.start()
    try:
...
def main() -> int:
    ...
    parser.add_argument("--level-calibrate", action="store_true", help="Average level offsets while still")
    ...
    if args.level_calibrate:
        return run_level_calibrate(args.bus, args.address, args.seconds)
    if args.spin_test:
